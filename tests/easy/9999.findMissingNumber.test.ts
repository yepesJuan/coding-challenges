...
describe("findMissingNums", () => {
  test("should return correct distinct integers from nums1 and nums2", () => {
    ...
    // Test case 2
    expect(findMissingNums([1, 2, 3, 3], [1, 1, 2, 2])).toEqual([[3], []]);

    // Test case 3: Completely distinct arrays
    expect(findMissingNums([1, 2, 3], [4, 5, 6])).toEqual([
      [1, 2, 3],
      [4, 5, 6],
    ]);

    // Test case 4: Identical arrays
    expect(findMissingNums([1, 2, 3], [1, 2, 3])).toEqual([[], []]);

    // Test case 5: One array is empty
    expect(findMissingNums([], [1, 2, 3])).toEqual([[], [1, 2, 3]]);
    expect(findMissingNums([1, 2, 3], [])).toEqual([[1, 2, 3], []]);
